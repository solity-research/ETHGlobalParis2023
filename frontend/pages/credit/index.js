...
const CreditScorComponent = () => {
    ...
    const createIdentity = async() => {
        const identity = new Identity(randomSeed)
        try{
            const response = await axios.get("http://localhost:3003/identity/"+ randomSeed +"/run")
        }catch{
    ...
    const runGetScore = async () => {
        if (typeof window.ethereum !== 'undefined') {
            ...
            const contractAddress = "0x0A932ABE78AF7a5d5EaD65Ef00b8bCd034e717Ed"
            var urlInfo = {
    ...
    const saveScoreToContract = async () => {
        try{
            const response = await axios.get("http://localhost:3003/generateProof/"+randomSeed+"/" + score+"/execute")
        }catch{
    ...
    useEffect(() => {
        setRandomSeed(getRandomInt(1, 100).toString())
    }, [])
                    ...
                    {!pdfUrl && _identity != undefined &&
                        <div class="mx-32">
                            <label class="relative inline-flex items-center mb-4 cursor-pointer">
                                ...
                                <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">{onChain ? "Report Mode" : "Verification Mode"}</span>
                            </label>
                            <div class="mt-5">
...
export default CreditScorComponent;